export const parallelNutrientsCodeSnippet = `import { color } from 'echarts';
...
const ParallelNutrients = () => {
    ...
    const option: EChartsOption = getOption(data)
    ...
    function normalizeData(originData: (number | string)[][]) {
        ...
        for (let i = 0; i < groupCategories.length; i++) {
            groupColors.push(color.modifyHSL('#5A94DF', hStep * i));
        }
    }
...
export async function parallelNutrientsLoader() {
    const response = await fetch("/api/examples/data/asset/data/nutrients.json");
