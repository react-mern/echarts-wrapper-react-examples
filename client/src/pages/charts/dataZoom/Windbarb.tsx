...
const Windbarb = () => {
  ...
  const renderArrow: CustomSeriesOption['renderItem'] = function (_param, api) {
  ...
  const renderWeather = function (_param: CustomSeriesRenderItemParams, api: CustomSeriesRenderItemAPI) {
    const point = api.coord([(api.value(dims.time) as number) + (3600 * 24 * 1000) / 2, 0]);
  ...
  const option: any = {
    ...
    tooltip: {
      ...
      formatter: function (params: any) {
        return [
          format.formatTime('yyyy-MM-dd', params[0].value[dims.time]) +
            ' ' +
            format.formatTime('hh:mm', params[0].value[dims.time]),
      ...
      },
      {
        ...
        tooltip: {
          ...
          formatter: function (param: any) {
            return param.value[dims.time] + ': ' + param.value[dims.minTemp] + ' - ' + param.value[dims.maxTemp] + '°';
          },
        },
        yAxisIndex: 2,
        z: 11,
...
export async function windbarbLoader() {
  const response = await fetch(
    import.meta.env.VITE_BASE_SERVER_URL + '/examples/data/asset/data/wind-barb-hobart.json',
  );
